...
object Client2 extends App {
  ...
  titles = "Hunted Down" :: Nil
  titles = "A Tale of Two Cities" :: titles
  titles = "The Cricket on the Hearth" :: titles
  
  println("Running the second mapper/reducer which compute the reverse index for proper names in a set of text files")
  init
  val mapfunc = new NameCountMap()
  setMapper(mapfunc)
  val reducefunc = new NameCountReduce()
  setReducer(reducefunc)
  for (i <- 0 until urls.size)
    master ! titles(i) + "\t" + urls(i)
  Thread.sleep(500)
  master ! Flush
  Thread.sleep(5000)
  system.terminate()
